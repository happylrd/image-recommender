...
爬取策略
1. 取300张热门图片，拿到250个不同的owner_id。
2. 对每个owner_id，取其所上传的150张图片id。
3. 对每个图片id，拿到具体的图片信息，进行一些处理（按照一定规则，跳过无意义数据；对每个图片id，拿到喜欢此图片的10个用户的信息，将其作为子字段加入图片信息对象中；删掉无用字段）。
4. 为每个用户生成一个json文件（文件名格式为`photo_{owner_id}`），包含该用户上传的n(n <= 150)张图片信息。
5. 按文件大小降序取前100个json文件作为入库数据。
